...
from . import status, server_ping
...
class ApiHandler:
    ...
    def handle_request(self, data: dict[str, Any]) -> dict:
        '''处理 API 请求，返回响应 dict'''
        if not isinstance(data, dict):
            return self._response(400, '', 'FAILED', 'request must be an object')
        ...
        if not isinstance(api_data, dict):
            return self._response(400, api, 'FAILED', 'data must be an object', echo=echo)

        handler = self._handlers.get(api)
    ...
    def _api_get_status(self, data: dict, echo: str) -> dict:
        ...
        try:
            ping_result = server_ping.get_server_list_ping(
                self.server,
